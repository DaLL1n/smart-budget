import {StrictMode} from 'react';
import {createRoot} from 'react-dom/client';
import App from './App.tsx';
import './index.css';

// Global capturing scroll tracker: illuminates custom scrollbars dynamically on mobile touch & desktop scrolling everywhere
if (typeof window !== 'undefined') {
  const triggerScrollingState = (target: EventTarget | null) => {
    if (!target || !(target instanceof HTMLElement)) return;

    let el: HTMLElement | null = target;
    while (el && el !== document.body && el !== document.documentElement) {
      if (
        el.scrollHeight > el.clientHeight ||
        el.scrollWidth > el.clientWidth ||
        el.classList.contains('custom-scrollbar')
      ) {
        el.setAttribute('data-scrolling', 'true');
        const timer = (el as any).__scrollTimer;
        if (timer) clearTimeout(timer);
        (el as any).__scrollTimer = setTimeout(() => {
          el?.removeAttribute('data-scrolling');
        }, 1000);
        return;
      }
      el = el.parentElement;
    }

    // If window/root level is scrolling
    document.documentElement.setAttribute('data-scrolling', 'true');
    const timer = (document.documentElement as any).__scrollTimer;
    if (timer) clearTimeout(timer);
    (document.documentElement as any).__scrollTimer = setTimeout(() => {
      document.documentElement.removeAttribute('data-scrolling');
    }, 1000);
  };

  ['scroll', 'touchmove', 'touchstart', 'wheel'].forEach((eventName) => {
    window.addEventListener(
      eventName,
      (e) => triggerScrollingState(e.target),
      { capture: true, passive: true }
    );
  });
}

createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <App />
  </StrictMode>,
);
